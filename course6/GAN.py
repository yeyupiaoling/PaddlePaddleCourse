...
# 生成真实图片reader
mnist_generator = paddle.batch(mnist_reader(paddle.dataset.mnist.train()), batch_size=128)
# 生成假图片的reader
...
for pass_id in range(20):
    for i, real_image in enumerate(mnist_generator()):
        ...
        r_real = exe.run(program=train_d_real,
                         ...
                         feed={real_img.name: np.array(real_image)})
        ...
        if i % 100 == 0:
            print("Pass：%d, Batch：%d, 训练判别器D识别真实图片Cost：%0.5f, "
                  ...
                  "训练生成器G生成符合判别器D标准的假图片Cost：%0.5f" %
                  (pass_id, i, r_fake[0], r_real[0], r_g[0]))
